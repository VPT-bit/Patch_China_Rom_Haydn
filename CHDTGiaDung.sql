...
SELECT MaSP, TenSP, DonGia, SoLuongTon, MaLoai, MaNCC
FROM SanPham
WHERE MaSP NOT IN (
    SELECT DISTINCT MaSP FROM ChiTietHoaDon
);
-- e3. Danh sách nhân viên chưa lập hóa đơn nào
SELECT MaNV, TenNV, GioiTinh, NgaySinh, DiaChi, SoDienThoai
FROM NhanVien
WHERE MaNV NOT IN (
...
WHERE MaLoai = (
    SELECT MaLoai FROM LoaiSanPham WHERE TenLoai = N'Tủ lạnh'
);
